...
class Config:
    ...
    # ── 稀疏纵向数据 (动态分支) 配置 ──
    static_id_col: str = "ID"           # 静态数据中的患者 ID 列
    dyn_id_col: str = "patient_id"      # 动态数据中的患者 ID 列
    # 动态跟踪覆盖的特征 (按你实际数据里有的 6 个变量调整)
    # 这些必须是 num_cols 的子集
    dyn_cols: tuple = ("C_G", "C_WBC", "C_RBC", "C_P", "C_N", "B_PCT")
...
class CSFDataset(Dataset):
    ...
    动态数据维度: [N, n_dyn_features, T]   (n_dyn_features = 6 个 CSF 变量)
    has_dyn_mask: [N], 0/1 显式标记每个样本是否有有效动态数据
    """
    def __init__(self, x_num, x_cat, labels=None, x_dynamic=None,
                 has_dyn_mask=None, n_dyn_features: int = 6, n_time_points: int = 4):
        ...
        if x_dynamic is None:
            # 没有动态数据: 用零张量填充
            self.x_dynamic = torch.zeros(N, n_dyn_features, n_time_points, dtype=torch.float32)
        else:
            self.x_dynamic = torch.tensor(x_dynamic, dtype=torch.float32)

        if has_dyn_mask is None:
            self.has_dyn = torch.zeros(N, dtype=torch.float32)
        else:
            self.has_dyn = torch.tensor(np.asarray(has_dyn_mask), dtype=torch.float32)
    ...
    def __getitem__(self, idx):
        ...
        return (self.x_num[idx], self.x_cat[idx], self.x_dynamic[idx],
                self.has_dyn[idx], self.labels[idx])
...
class TrendTokenEncoder(nn.Module):
    """
    输入: x_dyn [B, n_dyn_features, T]
        n_dyn_features = 动态跟踪覆盖的变量数 (例如 6 个 CSF 指标)
        与静态总特征数 (n_num_features = 21) 解耦
...
class DACTFormer(nn.Module):
    def __init__(
        ...
        n_time_points: int = 4,
        top_k_pairs: int = 24,
        n_dyn_features: int = 0,        # 动态分支覆盖的变量数 (如 6 个 CSF)
    ):
        super().__init__()
        ...
        self.use_dae = use_dae
        self.use_pda = use_pda
        # 只有当 use_tte=True 且实际有动态特征时才构造 TTE
        self.use_tte = use_tte and (n_dyn_features > 0)
        ...
        if self.use_tte:
            self.tte = TrendTokenEncoder(n_dyn_features, embed_dim, dropout=dropout)
...
def run_single_experiment(cfg: Config, df, num_cols, cat_cols, y,
                          ...
                          dynamic_array=None, has_dyn_mask=None):
    """
    动态数据相关参数:
      dynamic_array : np.ndarray of shape [N, n_dyn_features, T]
                      n_dyn_features 由 cfg.dyn_cols 决定 (例如 6)
      has_dyn_mask  : np.ndarray of shape [N], 0/1, 显式标记哪些样本有动态数据
    """
    ...
    # 定位动态特征在 num_cols 中的索引位置 (用于尺度同步)
    dyn_cols = list(cfg.dyn_cols)
    dyn_cols_in_num = [c for c in dyn_cols if c in num_cols]
    if dynamic_array is not None and len(dyn_cols_in_num) != len(dyn_cols):
        missing = [c for c in dyn_cols if c not in num_cols]
        print(f"  ⚠️ 警告: dyn_cols 中以下变量不在 num_cols 里: {missing}")
        print(f"     动态分支只会用到: {dyn_cols_in_num}")
    n_dyn_features = len(dyn_cols_in_num)
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(df, y), start=1):
        ...
        # 动态数据切片 + 标准化 (维度 = n_dyn_features, 不是 n_num_features)
        if dynamic_array is not None and n_dyn_features > 0:
            dyn_train = dynamic_array[train_idx].copy()
            dyn_val   = dynamic_array[val_idx].copy()
            has_dyn_train = has_dyn_mask[train_idx] if has_dyn_mask is not None else None
            has_dyn_val   = has_dyn_mask[val_idx]   if has_dyn_mask is not None else None

            # 用静态 scaler 同步标准化动态数据 (尺度一致)
            for fi, feat in enumerate(dyn_cols_in_num):
                static_idx = num_cols.index(feat)
                m = scaler.mean_[static_idx]
                s = scaler.scale_[static_idx]
                # 只对 has_dyn=1 的样本做标准化, has_dyn=0 的保持零张量
                if has_dyn_train is not None:
                    train_mask = has_dyn_train.astype(bool)
                    dyn_train[train_mask, fi, :] = (dyn_train[train_mask, fi, :] - m) / (s + 1e-6)
                else:
                    dyn_train[:, fi, :] = (dyn_train[:, fi, :] - m) / (s + 1e-6)
                if has_dyn_val is not None:
                    val_mask = has_dyn_val.astype(bool)
                    dyn_val[val_mask, fi, :] = (dyn_val[val_mask, fi, :] - m) / (s + 1e-6)
                else:
                    dyn_val[:, fi, :] = (dyn_val[:, fi, :] - m) / (s + 1e-6)

            # 训练集动态数据覆盖率统计 (论文里要报告)
            if has_dyn_train is not None:
                tr_cov = has_dyn_train.sum() / len(has_dyn_train)
                va_cov = has_dyn_val.sum()   / len(has_dyn_val)
                if fold == 1:
                    print(f"  动态数据覆盖率: 训练 {tr_cov*100:.1f}%, 验证 {va_cov*100:.1f}%")
        else:
            dyn_train = None
            dyn_val   = None
            has_dyn_train = None
        ...
        train_loader = DataLoader(
            CSFDataset(x_train_num, x_train_cat, y_train,
                       x_dynamic=dyn_train, has_dyn_mask=has_dyn_train,
        ...
        val_loader = DataLoader(
            CSFDataset(x_val_num, x_val_cat, y_val,
                       x_dynamic=dyn_val, has_dyn_mask=has_dyn_val,
                       n_dyn_features=n_dyn_features,
                       n_time_points=cfg.n_time_points),
        ...
        model = DACTFormer(
            ...
            n_dyn_features=n_dyn_features if dynamic_array is not None else 0,
        )
...
def grid_search(cfg: Config, df, num_cols, cat_cols, y,
    ...
    for i, combo in enumerate(all_combinations, start=1):
        ...
        summary, _ = run_single_experiment(
            ...
            dynamic_array=dynamic_array, has_dyn_mask=has_dyn_mask,
            exp_output_dir=None, save_fold_files=False,
        )
...
# 主函数
# =============================================================
def load_sparse_dynamic_data(static_path, dynamic_path, dyn_cols,
                              n_time_points=4,
                              static_id_col="ID", dyn_id_col="patient_id"):
    """
    加载稀疏纵向动态数据。

    动态 CSV 预期格式 (宽表):
      列: [patient_id, C_G_t0, C_G_t1, C_G_t2, C_G_t3, C_WBC_t0, ..., B_PCT_t3]
      每行一个有动态跟踪的病人 (~300 行)

    返回:
      df_static     : 原始静态 DataFrame
      dynamic_array : np.ndarray [N, len(dyn_cols), T], 无动态数据样本对应行全 0
      has_dyn_mask  : np.ndarray [N], 0/1
    """
    df_static = pd.read_excel(static_path)
    N = len(df_static)
    F = len(dyn_cols)
    T = n_time_points

    dynamic_array = np.zeros((N, F, T), dtype=np.float32)
    has_dyn_mask  = np.zeros(N, dtype=np.int8)

    if dynamic_path is None:
        return df_static, dynamic_array, has_dyn_mask

    df_dyn = pd.read_csv(dynamic_path)

    if static_id_col not in df_static.columns:
        raise ValueError(f"静态数据缺少 ID 列: {static_id_col}, 可用列: {list(df_static.columns)}")
    if dyn_id_col not in df_dyn.columns:
        raise ValueError(f"动态数据缺少 ID 列: {dyn_id_col}, 可用列: {list(df_dyn.columns)}")

    # 静态数据 ID → 行索引的映射
    id_to_idx = {pid: i for i, pid in enumerate(df_static[static_id_col].values)}

    matched, missing_ids = 0, 0
    ...
    print(f"  动态数据: 匹配 {matched}/{len(df_dyn)} 行 (在静态中找不到 ID 的: {missing_ids})")
    print(f"  覆盖率: {has_dyn_mask.sum()}/{N} ({100*has_dyn_mask.sum()/N:.1f}%) 静态样本有纵向跟踪")
    print(f"  动态变量数: {F} ({list(dyn_cols)})")
...
def main():
    ...
    print("加载数据...")
    df_static, dynamic_array, has_dyn_mask = load_sparse_dynamic_data(
        static_path  = CFG.data_path,
        dynamic_path = CFG.dynamic_data_path if CFG.use_tte else None,
        dyn_cols     = list(CFG.dyn_cols),
        n_time_points = CFG.n_time_points,
        static_id_col = CFG.static_id_col,
        dyn_id_col    = CFG.dyn_id_col,
    )

    df, num_cols, cat_cols, y = build_feature_dataframe(df_static, CFG)

    # 动态分支只在有数据时才启用
    has_any_dyn = has_dyn_mask.sum() > 0
    if not has_any_dyn:
        print("  ⚠️ 没有动态数据可用, TTE 分支自动关闭")
        dynamic_array = None
        has_dyn_mask = None

    print(f"\n样本数: {len(df)}, 阳性: {int(y.sum())}")
    print(f"数值特征: {len(num_cols)}, 类别特征: {len(cat_cols)}")
    print(f"消融开关: DAE={CFG.use_dae}, PDA={CFG.use_pda}, "
          f"TTE={CFG.use_tte and has_any_dyn}, "
          f"Focal={CFG.use_focal}, Mixup={CFG.use_mixup}")
    print(f"设备: {CFG.device}\n")

    best_row, _ = grid_search(CFG, df, num_cols, cat_cols, y,
                               dynamic_array=dynamic_array,
                               has_dyn_mask=has_dyn_mask)
