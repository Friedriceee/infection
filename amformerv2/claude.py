...
def run_single_experiment(cfg: Config, df, num_cols, cat_cols, y,
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(df, y), start=1):
        ...
        model_path = (
            exp_output_dir / f"best_model_fold{fold}_emb{cfg.embed_dim}_topk{cfg.top_k}.pth"
            if exp_output_dir
            else Path(f"temp_best_model_fold{fold}_emb{cfg.embed_dim}_topk{cfg.top_k}.pth")
)
...
def main():
    ...
    print("\n不进行网格搜索，直接使用当前固定参数：")
    print(f"top_k={best_cfg.top_k}, pos_weight_scale={best_cfg.pos_weight_scale}, embed_dim={best_cfg.embed_dim}")
